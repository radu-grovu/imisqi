...
export async function GET(req: Request) {
  ...
  if (secret !== process.env.CRON_SECRET) return new NextResponse('Forbidden', { status: 403 });
  ...
  for (const c of campaigns || []) {
    ...
    const idx = Math.floor((Date.parse(today) - Date.parse(c.start_date)) / (1000*60*60*24));
